...
public class SfntTool {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (option != null) {
                if (option.equals("help") || option.equals("?")) {
                    ...
                    if (!file.exists()) {
                        System.out.println("错误文件(夹)路径");
                        System.exit(1);
                    }

                    var fileTypes = new String[]{".htm", ".html", ".js", ".txt", ".java", ".xml"};
                    var files = allFiles(file).stream().filter(file1 -> {
                        var fileName = file1.getName();
                        var hasMatch = Arrays.stream(fileTypes).anyMatch(s -> fileName.endsWith(s));

                        return hasMatch;
                    }).toArray();

                    if (files.length == 0) {
                        System.out.println("未检索到可遍历文件");
                        System.exit(1);
                    }
                    ...
                    for (Object o : files) {
                        ...
                        lr.lines().forEach(new Consumer<String>() {
                            @Override
                            public void accept(String s) {
                                for (int i=0; i < s.length(); i++) {
                                    var c = s.substring(i, i+1);
                                    if (c.trim().length() != 0 && !sbuff.contains(c)) {
                                        sbuff += c;
                                    }
                                }
                            }
                    ...
                    if (sbuff.length() == 0) {
                        System.out.println("未检索可提取字符");
                        System.exit(1);
                    }

                    tool.subsetString = sbuff;
                    i++;
        ...
        if (file.isFile()) {
            ...
            for (File listFile : file.listFiles()) {
                if (listFile.isFile()) {
                    ...
                    List<File> subs = allFiles(listFile);
                    files.addAll(subs);
                }
            }
        }
    ...
    private static final void printUsage() {
        ...
        System.out.println("\t-f,-file\t 从文件(夹)提取字符串子集");
